...
input:string
output:string with all WUB's removed
explicit requirements:
  -given a string full of WUB's remove all of them and return the words in between with spaces 

datastructures
input: string// 'WUBIWUBLOVEWUBYOU'
output:string //'I LOVE YOU'

algorithm
swap out all the WUB's in the string with white spaces 
split each word to an array and get rid of all white space elements
join the array with single white space in between 
=end
def song_decoder(str)
 arr = str.gsub('WUB', ' ').strip.chars
 arr.delete(' ')
 arr.join(' ')
end
